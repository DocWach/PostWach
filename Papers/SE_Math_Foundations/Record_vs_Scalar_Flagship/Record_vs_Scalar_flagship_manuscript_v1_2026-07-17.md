...
A reduced model is a compression of a detailed one. Systems engineering and simulation carry that compression in two very different currencies. One is a scalar: a degree of homomorphism D_s that measures how far a proposed map departs from an exact structural correspondence [11], a lumping-error number for a Markov aggregation [17], a certified residual norm for a reduced basis [18]. The other is the *record*: the full list, block by block, of which fine states collapse to which coarse state and with what mass. A scalar is cheap to store and easy to compare; a record is expensive and unwieldy. The engineering instinct is to keep the scalar and discard the record.
...
In stochastic (Markov / DEVS) lumping, a fine chain is aggregated into blocks and the quantity of interest is an outcome distribution. The lumping-error scalars are LumpSTD (the max standard deviation of outgoing block probabilities, zero iff exactly lumpable), the structure-degree D_s, and, importantly, the *modern* structural discriminators: the Michel-Siegle partition-dependent transient bound [21] and the Geiger information-bottleneck KL-aggregation cost [22]. These are purpose-built to distinguish equal-size aggregations by structure, so they are the strongest legal scalars, not weak ones.
...
The entropy identity is Shannon and Landauer [5] [7]; the uniqueness kernel is Shannon-Khinchin-Faddeev [9] [10]; the per-scalar sufficiency test is Fisher-Neyman and data-processing [25] [6]. The stochastic-lumping machinery is Kemeny-Snell and Zeigler [17] [2], and the modern structural discriminators the record beats in M1 are Michel-Siegle and Geiger [21] [22]. The stronger scalars that defeat M2 and M3 are the certified reduced-basis energy norm [18] [26] and the label-aware kernel two-sample statistic [23], with the label-free impossibility due to Zhao and colleagues [24]. What is not in any of these as a unit is the specific combination this paper measures: the record-as-thermodynamic-carrier with three pairwise-distinct scalar summaries and a tower arrow (Section 2); the family-quantified three-regime boundary with faithful cross-modality classification (Section 3); and the located, certificate-tightness-gated delivery frontier (Sections 4 and 5).
...
- [17] J. G. Kemeny and J. L. Snell, *Finite Markov Chains*. Springer-Verlag, 1976. (`kemeny1976finite`)
- [18] G. Rozza, D. B. P. Huynh, and A. T. Patera, "Reduced Basis Approximation and a Posteriori Error Estimation for Affinely Parametrized Elliptic Coercive Partial Differential Equations: Application to Transport and Continuum Mechanics," *Arch. Comput. Methods Eng.*, vol. 15, no. 3, pp. 229--275, 2008. (`rozza2008reduced`)
- [19] T. Sagawa and M. Ueda, "Minimal Energy Cost for Thermodynamic Information Processing: Measurement and Information Erasure," *Phys. Rev. Lett.*, vol. 102, no. 25, p. 250602, 2009. (`sagawa2009minimal`)
- [20] P. Faist, F. Dupuis, J. Oppenheim, and R. Renner, "The Minimal Work Cost of Information Processing," *Nature Commun.*, vol. 6, art. 7669, 2015. (`faist2015minimal`)
- [21] F. Michel and M. Siegle, "Formal Error Bounds for the State Space Reduction of Markov Chains," *Perform. Eval.*, vol. 167, art. 102464, 2025. (`michel2024formal`)
- [22] B. C. Geiger, T. Petrov, G. Kubin, and H. Koeppl, "Optimal Kullback-Leibler Aggregation via Information Bottleneck," *IEEE Trans. Autom. Control*, vol. 60, no. 4, pp. 1010--1022, 2015. (`geiger2015optimal`)
- [23] A. Gretton, K. M. Borgwardt, M. J. Rasch, B. Scholkopf, and A. Smola, "A Kernel Two-Sample Test," *J. Mach. Learn. Res.*, vol. 13, pp. 723--773, 2012. (`gretton2012kernel`)
- [24] H. Zhao, R. Tachet des Combes, K. Zhang, and G. J. Gordon, "On Learning Invariant Representations for Domain Adaptation," in *Proc. 36th Int. Conf. Machine Learning*, PMLR 97, pp. 7523--7532, 2019. (`zhao2019learning`)
- [25] R. A. Fisher, "On the Mathematical Foundations of Theoretical Statistics," *Philos. Trans. R. Soc. A*, vol. 222, pp. 309--368, 1922. (`fisher1922mathematical`)
- [26] K. Veroy, C. Prud'homme, D. V. Rovas, and A. T. Patera, "A Posteriori Error Bounds for Reduced-Basis Approximation of Parametrized Noncoercive and Nonlinear Elliptic Partial Differential Equations," in *16th AIAA Computational Fluid Dynamics Conf.*, AIAA 2003-3847, 2003. (`veroy2003posteriori`)

All candidate references above were verified against authoritative sources (CrossRef + publishers) and promoted to the portfolio approved store on 2026-07-20; the in-text placeholder set is now empty. Additional verified-and-approved sources staged for potential citation but not yet cited in-text: Bennett 1982 (`bennett1982thermodynamics`), Szilard 1964 English translation (`szilard1964decrease`), Giles 2008 (`giles2008multilevel`), Courtois-Semal 1984 (`courtois1984bounds`), Buchholz 1994 (`buchholz1994exact`), Ben-David et al. 2010 (`bendavid2010theory`).
